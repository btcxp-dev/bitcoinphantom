...
// Copyright (c) 2018-2019, The Bitcoinphantom Developers
// 
...
int main(int argc, char **argv)
{
	/* Fix wallet not responding when enter cyrillic (non-latin) characters 
	   ...
	   Set cyrillic locale on Windows explicitly - this is Bitcoinphantomvanets after all. */
	setlocale(LC_CTYPE, "");
#ifdef WIN32
	SetConsoleCP(1251);
	...
	*/
	if (!node->feeAddress().empty()) {
		...
		feemsg << std::endl << "You have connected to a node that charges " <<
			...
			"but no more than " << formatAmount(10000000000000) << "BTCX." 
			<< std::endl << std::endl <<
